...
from mylib.runtime import DEFAULT_MODEL_RID, configure_token, load_model_from_source, run_fault_clearing_scan  # noqa: E402
...
def main() -> None:
    ...
    output_dir = root / "results" / "skill-verification" / "fault-clearing-scan"
    config = {
        "fault_point": {
            "fault_bus": None,
    ...
    result = run_fault_clearing_scan(model, config=config, output_dir=output_dir)
    if not result["results"]:
        raise RuntimeError("No clearing-scan results were produced")
    if "monotonic_degradation" not in result:
        raise RuntimeError("monotonic_degradation missing from result")
